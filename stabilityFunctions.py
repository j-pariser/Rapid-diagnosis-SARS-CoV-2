...
def extractConcentration(electrodeFolder, dataType):
    allFiles = os.listdir(electrodeFolder) # All the files in folder
    filesList = []
    for file in allFiles:  # Loop through each file in folder
        if file.endswith('.dta') or file.endswith('.DTA'):  # Only take DTA files
...
def plotFreq(electrodesFolders, dataType, electrodeNames, df):
    peaksMatrix = []
    for i, electrode in enumerate(electrodesFolders):
        #fileList = extractDataType(entry, dataType)
        fileList = extractConcentration(electrode, dataType)
    ...
    plt.plot(freq_list, peaksMatrix[0], marker = 'o', color='red' )
    plt.plot(freq_list, peaksMatrix[1], marker='o', color='green')
    plt.plot(freq_list, peaksMatrix[2], marker='o', color='blue')
    plt.title('SWV Frequency Map Without MCH', fontsize = '22')
    plt.xlabel('Frequencies (Hz)', fontsize = '15')
    plt.ylabel('Peak Current (nA)', fontsize = '15')
...
def peaks(filesList, electrodeFolder, method1, fraction1, method2, fraction2):
   ...
   while n < len(filesList):
    ...
    # Median Filter
       ...
       # # Find local "minimas" for baseline
       firstDeriv = np.gradient(smoothY)
       firstDeriv = scipy.signal.savgol_filter(firstDeriv, 19, 3)
       # plt.figure()
       # plt.title('Derivative of DPV %s Data' % filename)
       # plt.xlabel('Vfwd [V]')
       # plt.ylabel('Idif [A]')
...
def normalize(df_peaks,referenceMeasurement,experimentConditions):
    ...
    df_normal = pd.DataFrame(normal_pks)  # Convert matrix to dataframe
    return df_normal
...
def plotSignalStability(normSignal, electrodeNames):
    ...
    plt.title('Stability of Signal', fontsize=18)
    plt.ylabel('Normalized Signal Change [%]', fontsize='16')
    plt.xlabel('# of Measurements', fontsize='16')
...
def plotConcentration(averages, concentrations, electrodeNames,frequency):
    ...
    # print(dfy)
    plt.plot(concentrations, dfy, marker = 'o', markersize = '10', linestyle = '', color='m', markeredgecolor='k', markeredgewidth=1.0)  # plots graph
    plt.errorbar(concentrations, dfy, yerr = error, ecolor = 'k', linestyle = '', capsize = 4)
    plt.xlabel('Concentration S1 (fg/mL)', fontsize = '18')
    plt.ylabel('Normalized Signal Change (%)', fontsize = '18')  # gives y-label for bar graph
...
def plotPeaks(rawPeaks, concentrations, electrodeNames, frequency):
    ...
    plt.rcParams['xtick.labelsize'] = 15
    n = 0
    for i in range(len(rawPeaks.index)):
        plt.plot(concentrations, rawPeaks.iloc[i,:], '-o', label=electrodeNames[n])
        n+=1
    plt.xlabel('Concentration S1 (fg/mL)', fontsize = '15')
    plt.ylabel('Peak Height (nA)', fontsize = '15')  # gives y-label for bar graph
    plt.title('SWV Peak Height (%s)'% (frequency), fontsize='15', fontweight = 'bold')  # display title
